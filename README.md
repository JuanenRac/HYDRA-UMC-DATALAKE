...
**HYDRA-UMC-DATALAKE** is the long-term memory of the factory. It provides a scalable, high-performance repository for all time-series data generated by the ecosystem, including motor currents, joint angles, sensor readings, and AI inference logs.

It acts as the foundation for advanced analytics, predictive maintenance, and production optimization, enabling plant managers to look back at years of millisecond-accurate robotic performance.

### Key Features:
* 🗄️ **High-Resolution Storage:** Optimized for millions of points per second using InfluxDB/TimescaleDB.
...
* **Why sqlite3 today, not InfluxDB/TimescaleDB yet.** Both are named in this project's own badges/keywords, and remain the real long-term direction - but standing one up is a real infrastructure decision (a service to deploy and operate) that belongs to whoever runs this in production, not something to bolt on unasked. `src/hydra_umc_datalake/store.py`'s `TimeSeriesStore` is a genuinely real, ACID, queryable time-series store today (Python's stdlib `sqlite3`) - not a placeholder pretending to be one - kept behind its own class specifically so a real InfluxDB/TimescaleDB-backed implementation can replace it later. See `mejoras_futuras.txt`.
...
Pure-software service (ingestion/analytics integrator) - no hardware/firmware/os of its own, pruned from the template (see `SONNET/_papelera/` for ecosystem convention).
