...
def test_insert_rolls_back_on_a_mid_batch_failure_instead_of_leaving_a_dangling_transaction(
    ...
    # Sample's own dataclass typing is not enforced at runtime -
    ...
    # transaction, ready to be silently committed by whichever later,
    # unrelated insert() call happens to commit next - a real partial
    # write, just deferred rather than prevented.
    store.insert(Sample(source_id="robot-1", kind="motor_temp", timestamp=1000, fields={"value": 1.0}))
    assert store.sample_count() == 1

    bad = Sample(source_id={"nested": "robot-2"}, kind="motor_temp", timestamp=2000, fields={"value": 2.0})
    with pytest.raises(Exception):
        store.insert(bad)
    assert store.sample_count() == 1

    # The connection itself must still be usable afterwards - a dangling,
    # never-rolled-back transaction would otherwise corrupt or block this
    # next, entirely unrelated insert.
    store.insert(Sample(source_id="robot-3", kind="motor_temp", timestamp=3000, fields={"value": 3.0}))
    assert store.sample_count() == 2
